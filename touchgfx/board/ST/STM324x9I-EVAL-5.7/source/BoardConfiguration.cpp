...
static uint32_t frameBuf0 = (uint32_t)(0x64000000);//64
...
static void LCD_Config(void)
{
    ...
    RCC_PLLSAIConfig(192, 7, 2);//2
    RCC_LTDCCLKDivConfig(RCC_PLLSAIDivR_Div4);//4
    ...
    LTDC_Layer_InitStruct.LTDC_HorizontalStop = (640 + 144 - 1);
    LTDC_Layer_InitStruct.LTDC_VerticalStart = (LTDC_InitStruct.LTDC_AccumulatedVBP + 1);
    LTDC_Layer_InitStruct.LTDC_VerticalStop = (480 + LTDC_InitStruct.LTDC_AccumulatedVBP);
    ...
    */
    LTDC_Layer_InitStruct.LTDC_CFBLineLength = ((640 * 2) + 3);

    /*  the pitch is the increment from the start of one line of pixels to the
        start of the next line in bytes, then :
        Pitch = Active high width x number of bytes per pixel
    */
    LTDC_Layer_InitStruct.LTDC_CFBPitch = (640 * 2);
...
void touchgfx_init()
{
    HAL& hal = touchgfx_generic_init<STM32F4HAL>(dma, display, tc, 640, 480, (uint16_t*)(frameBuf0 + 640 * 480 * 2 * 3), 7000000);
    hal.setFrameBufferStartAddress((uint16_t*)frameBuf0);
    hal.setTouchSampleRate(2);
    hal.setFingerSize(1);
    dma.setDeadTime(0xa1); //insert DMA delays to avoid TFT exhaustion on this board.
