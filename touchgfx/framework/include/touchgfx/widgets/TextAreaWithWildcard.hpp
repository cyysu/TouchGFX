...
public:
    ...
    /**
     * @fn int16_t TextAreaWithWildcardBase::calculateTextHeight(const Unicode::UnicodeChar* format, ...);
     *
     ...
     */
    int16_t calculateTextHeight(const Unicode::UnicodeChar* format, ...);
};
...
public:
    ...
    virtual void draw(const Rect& area) const
    {
        if (typedText.hasValidId())
        {
            ...
            if (fontToDraw != 0)
            {
                LCD::StringVisuals visuals(fontToDraw, color, alpha, typedText.getAlignment(), linespace, rotation, typedText.getTextDirection(), indentation);
