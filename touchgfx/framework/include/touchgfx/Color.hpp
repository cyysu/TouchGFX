...
public:
    ...
    {
        return (HAL::lcd().bitDepth() == 16) ? ((color & 0xF800) >> 8) : ((color.getColor32() >> 16) & 0xFF);
    }
    ...
    {
        return (HAL::lcd().bitDepth() == 16) ? ((color & 0x07E0) >> 3) : ((color.getColor32() >> 8) & 0xFF);
    }
    ...
    {
        return (HAL::lcd().bitDepth() == 16) ? ((color & 0x001F) << 3) : (color.getColor32() & 0xFF);
    }
};
