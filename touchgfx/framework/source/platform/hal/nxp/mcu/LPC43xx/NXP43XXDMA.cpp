...
NXP43XXDMA::NXP43XXDMA(GPDMA_LLI_Type* lliMemory)
...
NXP43XXDMA::~NXP43XXDMA()
{
}


void NXP43XXDMA::initialize()
{
}
...
void NXP43XXDMA::signalDMAInterrupt(void)
{
    if (LPC_GPDMA->INTTCSTAT & 1)
    {
        LPC_GPDMA->INTTCCLEAR = 1;
        linecount++;
        if ((LPC_GPDMA->INTSTAT == 0) && LPC_GPDMACH0->CLLI == 0)
        {
            completedcount++;
            if (completedcount == 1)
            {
...
void NXP43XXDMA::setupDataCopy(const touchgfx::BlitOp& blitOp)
{
    ...
    for (; i < blitOp.nLoops - 1; i++)
    {
        ...
        {
            MyLLI[i].NextLLI = 0;
            MyLLI[i].Control |= GPDMA_DMACCxControl_I;
        }
        else
        {
...
void NXP43XXDMA::setupDataFill(const touchgfx::BlitOp& blitOp)
{
    numLines = blitOp.nLoops;
    while (LPC_GPDMA->ENBLDCHNS) ;
    GPDMA_Channel_CFG_Type GPDMACfg;
