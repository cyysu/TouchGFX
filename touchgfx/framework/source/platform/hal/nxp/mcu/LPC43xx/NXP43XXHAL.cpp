...
uint16_t* NXP43XXHAL::getTFTFrameBuffer() const
{
    return (uint16_t*)LPC_LCD->UPBASE;
}

void NXP43XXHAL::setTFTFrameBuffer(uint16_t* adr)
{
    LPC_LCD->UPBASE = (uint32_t)adr;
}

void NXP43XXHAL::configureInterrupts()
{
    NVIC_SetPriority(DMA_IRQn, 9);
    NVIC_SetPriority(LCD_IRQn, 9);
}

void NXP43XXHAL::disableInterrupts()
{
    NVIC_DisableIRQ(DMA_IRQn);
    NVIC_DisableIRQ(LCD_IRQn);
}

void NXP43XXHAL::enableInterrupts()
{
    NVIC_EnableIRQ(DMA_IRQn);
    NVIC_EnableIRQ(LCD_IRQn);
}

void NXP43XXHAL::enableLCDControllerInterrupt()
{
    LPC_LCD->INTMSK = 8;
}
