...
void JogWheel::handleDragEvent(const DragEvent& evt)
{
    ...
    float length = sqrtf((float)(vx * vx + vy * vy));

    //Calc dot product and angle diff
    int16_t dotProduct = (oldX * vx + oldY * vy);

    float angleDiff = acosf(dotProduct / (oldLength * length));
    ...
    int16_t newValue = startValue + stepInterval * ((int16_t)((degreesRotated / (3.14 * 2 * ANGLE_MULTIPLIER)) * deltaValueForOneRound));
