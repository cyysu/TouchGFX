...
{
    insert(&(ground.image1), player);
}

void ParallaxBackground::addCollectibleItem(CollectibleItem& item)
{
    insert(&(ground.image1), item);
}

// 1 = execute tick each handleTickEvent, x = execute only each x tick
void ParallaxBackground::setActiveTickInterval(int interval)
{
