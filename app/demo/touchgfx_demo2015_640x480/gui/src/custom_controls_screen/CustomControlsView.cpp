    ...
    tickCounter(0)
{
    ...
    slideMenu.setPosition(0, 0, 120, getActiveDisplayHeight());
    ...
    statusMessage.setPosition(background.getX() + (background.getWidth() - 300) / 2, 430, 300, 24);
    statusMessage.setColor(Color::getColorFrom24BitRGB(0xFF, 0xFF, 0xFF));
    statusMessage.setVisible(false);
    add(statusMessage);

    mcuLoadTxt.setXY(514, 460 - mcuLoadTxt.getTypedText().getFont()->getFontHeight());
    ...
    mcuLoadValueTxt.setColor(Color::getColorFrom24BitRGB(0x17, 0x3C, 0x51));
    add(mcuLoadTxt);
    add(mcuLoadValueTxt);
