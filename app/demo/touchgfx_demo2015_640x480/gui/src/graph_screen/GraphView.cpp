...
void GraphView::setupScreen()
{
    ...
    for (int i = 0; i < NUMBER_OF_GRID_LINES; i++)
    {
        ...
        graphGridLines[i].setPosition(graphXOffset, 60 + 60 * i, 575, 1);
    ...
    pieChart.setPosition(20, -46, 400, 400);
    ...
    primaryGraphLineSetting.setXY(20, 338);
    ...
    secondaryGraphLineSetting.setXY(primaryGraphLineSetting.getX(), primaryGraphLineSetting.getRect().bottom() + 8);
    ...
    gotoMenuButton.setXY(564, 406);
    add(gotoMenuButton);

    mcuLoadTxt.setXY(464, gotoMenuButton.getRect().bottom() - 24 - mcuLoadTxt.getTypedText().getFont()->getFontHeight());
        ...
        if ((tickCount % 1) == 0)
        {
            ...
            case 0:
                if (pointCounter < 40)
                {
                    ...
                    /*
                    if (leftX+13 + 400 < graphX)
                    {
                        primaryGraph.deleteValue(leftX);
                        secondaryGraph.deleteValue(leftX);
                        leftX += 13;
                        primaryGraph.setRange(leftX, leftX+400, 400, 0);
                        secondaryGraph.setRange(leftX, leftX+400, 400, 0);
                    }
                    */
                    graphX += 15;
...
void GraphView::buttonPressedHandler(const AbstractButton& button)
{
    ...
    else if (&button == &graphOrPieButton)
    {
        if (currentState == PIE_CHART)
        {
            ...
            pieChart.startMoveAnimation(20, -46, 16, EasingEquations::linearEaseIn);
            pieChart.activatePieChartAnimationEndedCallback(true);
        ...
        else if (currentState == GRAPH)
        {
            ...
            for (int i = 0; i < NUMBER_OF_PIE_CHART_SECTIONS; i++)
            {
                ...
                legendEntries[i].startMoveAnimation(440, legendEntries[i].getY(), 16, EasingEquations::cubicEaseInOut);
            }
            ...
            pieChart.setMoveAnimationDelay(0);
            pieChart.startMoveAnimation(20, -46, 35, EasingEquations::linearEaseOut);
            pieChart.activatePieChartAnimationEndedCallback(true);
    ...
    else if (&button == &donutOrPieButton)
    {
        if (pieChartIsFull)
        {
            ...
            pieChart.startMoveAnimation(20, 44, 35, EasingEquations::quintEaseInOut);
        }
        else
        {
            // To full circle
            ...
            pieChart.setMoveAnimationDelay(0);
            pieChart.startMoveAnimation(20, -46, 35, EasingEquations::linearEaseInOut);
        }
