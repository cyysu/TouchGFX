...
void MainMenuAnimatingButtonsView::setupScreen()
{
    ...
    startUpAnimationBox.setPosition(getActiveDisplayWidth() / 2, getActiveDisplayHeight() / 2, 0, 0);
    startUpAnimationBox.setAnimationEndedCallback(startUpAnimationBoxDone);
    add(startUpAnimationBox);

    for (int i = 0; i < NUMBER_OF_BUTTONS; i++)
    {
        buttonContainers[i].setPosition((getActiveDisplayWidth() - BUTTONS_WIDTH) / 2, ((getActiveDisplayHeight() - (BUTTONS_HEIGHT * NUMBER_OF_BUTTONS)) / 2) + i * BUTTONS_HEIGHT, BUTTONS_WIDTH, BUTTONS_HEIGHT);
...
void MainMenuAnimatingButtonsView::buttonClickedHandler(const AbstractButton& button)
{
    ...
    for (int i = 0; i < NUMBER_OF_BUTTONS; i++)
    {
        ...
        buttonContainers[i].startMoveAnimation((getActiveDisplayWidth() - BUTTONS_WIDTH) / 2, ((getActiveDisplayHeight() - (BUTTONS_HEIGHT * NUMBER_OF_BUTTONS)) / 2) + i * BUTTONS_HEIGHT, 16, EasingEquations::linearEaseNone, EasingEquations::backEaseOut);
    }
...
void MainMenuAnimatingButtonsView::buttonAnimationDoneHandler(const MoveAnimator<Container>& box)
{
    if (currentAnimationState == ANIMATE_BUTTONS_FROM_BOX)
    {
        for (int i = 0; i < NUMBER_OF_BUTTONS; i++)
        {
            ...
            texts[i].setXY(buttons[i].getRect().right(), buttons[i].getY() + 16);
