...
void ProductPresenterView::setupScreen()
{
    ...
    for (int i = 0; i < NUMBER_OF_POSTERS; i++)
    {
        ...
        poster[i].setXY(272, 0);
        posters[i].add(poster[i]);
        posters[i].setPosition(0, 0, 272 + poster[i].getWidth() + 80, poster[i].getHeight());
        posterSwipeContainer.add(posters[i]);
    }

    posterSwipeContainer.setXY(0, 50);
    posterSwipeContainer.setSwipeCutoff(80);
    posterSwipeContainer.setEndSwipeElasticWidth(40);
    ...
    activateLanguageSelectorButton.setXY(577, 434);
    ...
    languageSelector.setXY(628 - languageSelector.getWidth(), 429);
    ...
    gotoMenuButton.setWidth(gotoMenuButton.getWidth() + 30); // Increase the active area of the button to make it easier to hit
    gotoMenuButton.setHeight(gotoMenuButton.getHeight() + 30);
    add(gotoMenuButton);

    mcuLoadTxt.setXY(514, 37 - mcuLoadTxt.getTypedText().getFont()->getFontHeight());
